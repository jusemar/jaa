...
const ctx = criarAmbienteIntegracao({
  telefones: ["+5531987653001", "+5531987653002", "+5531987653003", "+5531987653004", "+5531987653005", "+5531987653006"],
  prefixoIp: "198.18.16.",
});
...
async function fecharTodasAsFormacoes() {
  for (const saida of await listarSaidas()) {
    if (saida.status === "em_formacao") await ctx.api(A, "POST", `/empresas/${pizzaria.id}/saidas/${saida.id}/fechar`);
  }
}

const listarSaidas = async (): Promise<SaidaEntrega[]> => ((await ctx.api(A, "GET", `/empresas/${pizzaria.id}/saidas`)).json() as ListaSaidas).saidas;
const saidaDoPedido = async (pedidoId: string) => (await listarSaidas()).find((saida) => saida.paradas.some((parada) => parada.pedidoId === pedidoId)) ?? null;
...
async function sairDaBase(pessoa: Pessoa, entregadorId: string) {
  for (let i = 0; i < 2; i++) {
    await ctx.api(pessoa, "POST", `/entregas/vinculos/${entregadorId}/localizacao`, {
      latitude: -19.99,
      longitude: -43.99,
      precisaoMetros: 10,
      medidaEm: new Date().toISOString(),
...
before(async () => {
  ...
  pizzaria = (await ctx.api(A, "POST", "/empresas", { nome: "Pizzaria Zona", nomeUsuario: `${PREFIXO}_pizza`, slug: `${PREFIXO}-pizzaria` })).json();
  padaria = (await ctx.api(A, "POST", "/empresas", { nome: "Padaria Zona", nomeUsuario: `${PREFIXO}_pada`, slug: `${PREFIXO}-padaria` })).json();
  pizza = (await ctx.api(A, "POST", `/empresas/${pizzaria.id}/produtos`, { nome: "Pizza Calabresa", precoCentavos: 3990 })).json();

  for (const cliente of [B1, B2, B3]) conversas.set(cliente.identidadeId, await ctx.abrirConversa(cliente, `${PREFIXO}_pizza`));
...
describe("zonas de entrega", () => {
  it("a empresa desenha o polígono e a zona nasce ativa", async () => {
    const resposta = await ctx.api(A, "POST", `/empresas/${pizzaria.id}/zonas`, { nome: "Zona A", vertices: ZONA_A });
    assert.equal(resposta.statusCode, 201, resposta.body);
    zonaA = resposta.json();
    assert.equal(zonaA.ativa, true);
    assert.equal(zonaA.vertices.length, 4);

    const respostaB = await ctx.api(A, "POST", `/empresas/${pizzaria.id}/zonas`, { nome: "Zona B", vertices: ZONA_B });
    ...
    assert.equal(((await ctx.api(A, "GET", `/empresas/${pizzaria.id}/zonas`)).json() as ListaZonas).zonas.length, 2);
  });

  it("recusa polígono inválido e área sobreposta a outra zona ativa", async () => {
    const poucosPontos = await ctx.api(A, "POST", `/empresas/${pizzaria.id}/zonas`, { nome: "Curta", vertices: ZONA_A.slice(0, 2) });
    ...
    const sobreposta = await ctx.api(A, "POST", `/empresas/${pizzaria.id}/zonas`, {
      nome: "Invasora",
      vertices: [
    ...
    assert.ok(String(sobreposta.json().mensagem).includes("Zona A"), "a mensagem diz qual zona conflita");

    const nomeRepetido = await ctx.api(A, "POST", `/empresas/${pizzaria.id}/zonas`, { nome: "zona a", vertices: [
  ...
  it("zona é da empresa: outra empresa e o cliente não leem nem editam", async () => {
    assert.equal((await ctx.api(B1, "GET", `/empresas/${pizzaria.id}/zonas`)).statusCode, 404);
    assert.equal((await ctx.api(P, "GET", `/empresas/${pizzaria.id}/zonas`)).statusCode, 404, "entregador não é administrador");
    assert.equal((await ctx.api(A, "PATCH", `/empresas/${padaria.id}/zonas/${zonaA.id}`, { nome: "Roubada", vertices: ZONA_A })).statusCode, 404);
    assert.equal(((await ctx.api(A, "GET", `/empresas/${padaria.id}/zonas`)).json() as ListaZonas).zonas.length, 0);
  });

  it("editar nome/desenho e desativar continuam disponíveis", async () => {
    const renomeada = await ctx.api(A, "PATCH", `/empresas/${pizzaria.id}/zonas/${zonaB.id}`, { nome: "Zona B (leste)", vertices: ZONA_B });
    assert.equal(renomeada.statusCode, 200, renomeada.body);
    assert.equal((renomeada.json() as ZonaEntrega).nome, "Zona B (leste)");

    const desativada = await ctx.api(A, "PATCH", `/empresas/${pizzaria.id}/zonas/${zonaB.id}`, { nome: "Zona B (leste)", vertices: ZONA_B, ativa: false });
    assert.equal((desativada.json() as ZonaEntrega).ativa, false);
    // Volta a ativa para os testes seguintes.
    zonaB = (await ctx.api(A, "PATCH", `/empresas/${pizzaria.id}/zonas/${zonaB.id}`, { nome: "Zona B (leste)", vertices: ZONA_B, ativa: true })).json();
  });
});

describe("formação automática", () => {
  it("pedido pronto entra na formação da zona dele; o segundo entra na MESMA saída", async () => {
    await ctx.api(A, "POST", `/empresas/${pizzaria.id}/despacho`, { maxPedidosPorSaida: 3, tempoFormacaoMinutos: 15, combinarZonas: false });
    ...
    assert.equal(saida.entregador, null, "em formação ainda não tem entregador");
    ...
    assert.equal(mesma?.id, saida.id, "não abre uma segunda saída da mesma zona à toa");
    assert.equal(mesma?.paradas.length, 2);
    // Entrar em formação NÃO avança o pedido: ele continua pronto.
    assert.equal((await ctx.api(A, "GET", `/empresas/${pizzaria.id}/pedidos/${primeiro.id}`)).json().status, "pronto");
  });
  ...
  it("ao atingir a quantidade máxima a saída fecha sozinha e a próxima começa vazia", async () => {
    ...
    assert.notEqual(nova?.id, fechada?.id, "o pedido seguinte abre uma NOVA formação da zona");
    assert.equal(nova?.status, "em_formacao");
    assert.equal(nova?.paradas.length, 1);
    // Nunca ultrapassa o limite configurado.
    for (const saida of await listarSaidas()) assert.ok(saida.paradas.filter((parada) => parada.encerradaEm === null).length <= 3, saida.id);
  });

  it("pedido fora de todas as zonas não entra na automação e aparece como pendência", async () => {
    const foraDeZona = await pedidoProntoEm(B3, PONTO_FORA);
    assert.equal(await saidaDoPedido(foraDeZona.id), null, "não foi encaixado em zona nenhuma");

    const painel: PainelDespacho = (await ctx.api(A, "GET", `/empresas/${pizzaria.id}/despacho`)).json();
    assert.equal(painel.automacaoAtiva, true);
    assert.ok(painel.pedidosForaDeZona.some((pedido) => pedido.id === foraDeZona.id), "a empresa vê a pendência");
    // Continua tratável à mão: o pedido segue pronto e sem saída.
    assert.equal((await ctx.api(A, "GET", `/empresas/${pizzaria.id}/pedidos/${foraDeZona.id}`)).json().status, "pronto");
  });
});

describe("fechamento por tempo e combinação de zonas", () => {
  it("sem combinação autorizada, a saída vencida fecha sozinha", async () => {
    await ctx.api(A, "POST", `/empresas/${pizzaria.id}/despacho`, { maxPedidosPorSaida: 5, tempoFormacaoMinutos: 15, combinarZonas: false });
    await fecharTodasAsFormacoes();
  ...
  it("com combinação autorizada, zonas compatíveis viram uma saída só — sem passar do limite", async () => {
    await ctx.api(A, "POST", `/empresas/${pizzaria.id}/despacho`, { maxPedidosPorSaida: 3, tempoFormacaoMinutos: 15, combinarZonas: true });
    await fecharTodasAsFormacoes();
    const compatibilidade = await ctx.api(A, "POST", `/empresas/${pizzaria.id}/zonas/${zonaA.id}/compatibilidades`, { zonaIds: [zonaB.id] });
    ...
    assert.deepEqual(zonas.find((zona) => zona.id === zonaB.id)?.compativeisCom, [zonaA.id]);
    ...
    assert.equal(combinada?.paradas.filter((parada) => parada.encerradaEm === null).length, 3, "encheu até o máximo configurado");
    assert.ok(combinada?.zonasCombinadas.some((zona) => zona.id === zonaB.id), "a saída mostra a zona combinada");
    assert.equal((await saidaDoPedido(naZonaB1.id))?.id, combinada?.id, "o pedido mais antigo de B foi o que entrou");
  ...
  it("zonas incompatíveis nunca são combinadas", async () => {
    await ctx.api(A, "POST", `/empresas/${pizzaria.id}/zonas/${zonaA.id}/compatibilidades`, { zonaIds: [] });
    await fecharTodasAsFormacoes();
    ...
    assert.equal(depois?.paradas.length, 1, "ficou sozinha: a compatibilidade foi removida");
    assert.deepEqual(depois?.zonasCombinadas, []);
    assert.equal((await saidaDoPedido(naZonaB.id))?.id, formacaoB?.id, "a zona B continua com o pedido dela");
  });
  ...
  it("o prazo vencido é reconhecido depois de um restart (ele vive no banco)", async () => {
    ...
    const fechadas = await processarFormacoesVencidas({ banco: ctx.banco, eventosEntregas: ctx.eventosEntregas });
    assert.ok(fechadas.includes(formacao?.id ?? ""), "o vencimento não se perde com o processo reiniciado");
  });
});
...
describe("despacho pelo primeiro entregador apto da fila", () => {
  it("quem está fora da base ou inapto nunca é escolhido; sem ninguém, a saída aguarda", async () => {
    ...
    assert.equal(aguardando?.entregador, null, "ninguém fora da base é escolhido");
  ...
  it("entregador entra na fila e a saída pendente é despachada automaticamente", async () => {
    ...
    await aguardarAte(() => eventos.some((evento) => evento.saida.id === atribuida.id));
    // Recebeu saída: saiu da fila da base.
    const situacoes = (await ctx.api(P, "GET", "/entregas/situacao")).json();
    assert.equal(situacoes.situacoes.find((item: { entregadorId: string }) => item.entregadorId === paulo).posicaoFila, null);
  });
  ...
  it("duas saídas fechando não recebem o mesmo entregador; a mais antiga vai primeiro", async () => {
    ...
    const pendentes = (await listarSaidas()).filter((saida) => saida.status === "aguardando_entregador");
    ...
    assert.ok((await listarSaidas()).some((saida) => saida.status === "aguardando_entregador"));
  });
  ...
  it("entregador de outra empresa nunca é usado, e o cliente não vê nada do despacho", async () => {
    ...
    assert.equal((await ctx.api(A, "GET", `/empresas/${padaria.id}/despacho`)).json().automacaoAtiva, false);
    const despachadasNaPadaria = await despacharPendentes(despacho(), padaria.id);
    assert.deepEqual(despachadasNaPadaria, [], "a padaria não despacha saída da pizzaria");

    assert.equal((await ctx.api(B1, "GET", `/empresas/${pizzaria.id}/despacho`)).statusCode, 404);
    assert.equal((await ctx.api(B1, "GET", `/empresas/${pizzaria.id}/saidas`)).statusCode, 404);
    assert.equal((await ctx.api(P, "GET", `/empresas/${pizzaria.id}/despacho`)).statusCode, 404);
  });
});
...
describe("controle do gestor e regra absoluta da saída iniciada", () => {
  ...
  it("saída iniciada NUNCA recebe pedido novo, nem da mesma zona", async () => {
    ...
    assert.notEqual(saidaDoNovo?.id, emAndamento.id, "o pedido novo foi para outra saída");

    const depois = (await listarSaidas()).find((saida) => saida.id === emAndamento.id);
    assert.equal(depois?.paradas.length, paradasAntes, "a saída iniciada não mudou");
    // Os pedidos daquela saída avançaram pela máquina de estados de sempre.
    assert.equal((await ctx.api(A, "GET", `/empresas/${pizzaria.id}/pedidos/${emAndamento.paradas[0]?.pedidoId}`)).json().status, "saiu_para_entrega");
  });

  it("pedido cancelado sai da formação e a saída continua consistente", async () => {
    const pedido = await pedidoProntoEm(B2, PONTO_B);
    const formacao = await saidaDoPedido(pedido.id);
    const quantidadeAntes = formacao?.paradas.filter((parada) => parada.encerradaEm === null).length ?? 0;

    const cancelado = await ctx.api(A, "POST", `/empresas/${pizzaria.id}/pedidos/${pedido.id}/cancelar`, { statusAtual: "pronto", motivo: "Cliente desistiu" });
    ...
    const depois: SaidaEntrega = (await ctx.api(A, "GET", `/empresas/${pizzaria.id}/saidas/${formacao?.id}`)).json();
    const ativas = depois.paradas.filter((parada) => parada.encerradaEm === null).length;
    ...
    else assert.equal(depois.status, "em_formacao", "com pedido restante, ela continua formando");
  });
});
