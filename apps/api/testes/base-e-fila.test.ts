...
import { entregadoresEmpresa, historicoFilaEntregador } from "@jaa/banco/schema";
import {
...
const ctx = criarAmbienteIntegracao({
  telefones: ["+5531987652101", "+5531987652102", "+5531987652103", "+5531987652104", "+5531987652105"],
  prefixoIp: "198.18.16.",
});
...
const enviarLocalizacao = (pessoa: Pessoa, entregadorId: string, coordenadas: { latitude: number; longitude: number }, extras: Record<string, unknown> = {}) =>
...
async function chegarNaBase(pessoa: Pessoa, entregadorId: string): Promise<SituacaoOperacional> {
...
async function sairDaBase(pessoa: Pessoa, entregadorId: string): Promise<SituacaoOperacional> {
...
const painel = async (empresa = pizzaria): Promise<PainelOperacional> => (await ctx.api(A, "GET", `/empresas/${empresa.id}/operacao`)).json();
...
before(async () => {
  ...
  assert.equal((await ctx.api(A, "POST", `/empresas/${pizzaria.id}/base/localizacao`, BASE)).statusCode, 200);
});
...
describe("base operacional da empresa", () => {
  it("guarda o endereço, o raio e só confirma o ponto por ação explícita", async () => {
    const base: BaseEmpresa = (await ctx.api(A, "GET", `/empresas/${pizzaria.id}/base`)).json();
    ...
    assert.equal((await ctx.api(A, "POST", `/empresas/${pizzaria.id}/base`, { cep: "30112-000", logradouro: "Av", numero: "1", bairro: "B", cidade: "BH", uf: "MG", raioMetros: 5 })).statusCode, 400);
  ...
  it("mudar o endereço estrutural invalida o ponto confirmado", async () => {
    ...
    assert.equal((await ctx.api(A, "POST", `/empresas/${outra.id}/base/localizacao`, BASE)).statusCode, 200);

    const editada: BaseEmpresa = (await ctx.api(A, "POST", `/empresas/${outra.id}/base`, { cep: "30112-000", logradouro: "Rua A", numero: "999", bairro: "Centro", cidade: "BH", uf: "MG" })).json();
    assert.equal(editada.numero, "999");
    assert.equal(editada.localizacaoConfirmadaEm, null, "outro número pode ser outro lugar: confirma de novo");
    // Só o raio muda? A confirmação continua valendo.
    assert.equal((await ctx.api(A, "POST", `/empresas/${outra.id}/base/localizacao`, BASE)).statusCode, 200);
    const soRaio: BaseEmpresa = (await ctx.api(A, "POST", `/empresas/${outra.id}/base`, { cep: "30112-000", logradouro: "Rua A", numero: "999", bairro: "Centro", cidade: "BH", uf: "MG", raioMetros: 300 })).json();
  ...
  it("só quem administra a empresa configura a base", async () => {
    for (const pessoa of [P, B]) {
      assert.equal((await ctx.api(pessoa, "GET", `/empresas/${pizzaria.id}/base`)).statusCode, 404, "entregador e cliente não veem a base");
      assert.equal((await ctx.api(pessoa, "POST", `/empresas/${pizzaria.id}/base`, { cep: "30112-000", logradouro: "X", numero: "1", bairro: "B", cidade: "BH", uf: "MG" })).statusCode, 404);
    }
    assert.equal((await ctx.api(null, "GET", `/empresas/${pizzaria.id}/base`)).statusCode, 401);
  });
});
...
describe("presença por geofence", () => {
  it("o servidor decide presença; o entregador nunca declara que está na base", async () => {
    ...
    const tentativa = await enviarLocalizacao(P, paulo, LONGE, { naBase: true });
  ...
  it("uma leitura precisa de confirmação: presença não muda no primeiro ponto", async () => {
    ...
    assert.equal(primeira.json().naBase, false, "uma leitura só não confirma chegada");
  ...
  it("oscilação do GPS perto da borda não tira ninguém da fila", async () => {
    ...
    const antes: SituacaoOperacional = (await ctx.api(C, "GET", "/entregas/situacao")).json().situacoes.find((item: SituacaoOperacional) => item.entregadorId === carlos);
    ...
    assert.equal(oscilacao.json().naBase, true, "ruído perto da borda não derruba a presença");
  ...
  it("recusa leitura antiga, imprecisa, de outra pessoa ou sem base confirmada", async () => {
    const antiga = await enviarLocalizacao(P, paulo, NA_BASE, { medidaEm: new Date(Date.now() - 10 * 60 * 1000).toISOString() });
    assert.equal(antiga.statusCode, 409);
    assert.equal(antiga.json().codigo, "LOCALIZACAO_IMPRECISA");
    const imprecisa = await enviarLocalizacao(P, paulo, NA_BASE, { precisaoMetros: 900 });
    ...
    assert.equal((await ctx.api(null, "POST", `/entregas/vinculos/${paulo}/localizacao`, { latitude: -19.93, longitude: -43.93, medidaEm: new Date().toISOString() })).statusCode, 401);

    // Empresa sem ponto de base confirmado não detecta presença.
    const naFarmacia = await ctx.criarEntregadorAtivo(A, farmacia.id, P, `${PREFIXO}_p`);
...
describe("fila automática", () => {
  it("entra sozinha, na ordem de chegada, sem gestor confirmar nada", async () => {
    for (const [pessoa, vinculo] of [[P, paulo], [C, carlos], [J, joao]] as const) await sairDaBase(pessoa, vinculo);
    ...
    assert.deepEqual(quadro.fila.map((item) => item.pessoa.nomeExibicao), ["Paulo Entregador", "Carlos Entregador", "João Entregador"]);
    assert.deepEqual(quadro.fila.map((item) => item.posicaoFila), [1, 2, 3]);
    assert.equal(quadro.fila.every((item) => item.estado === "disponivel_na_base"), true);
  ...
  it("separa fila, disponíveis fora da base e indisponíveis", async () => {
    ...
    const quadro = await painel();
    assert.deepEqual(quadro.fila.map((item) => item.pessoa.nomeExibicao), ["Paulo Entregador"]);
    assert.ok(quadro.foraDaBase.some((item) => item.pessoa.nomeExibicao === "João Entregador" && item.estado === "disponivel_fora_base"));
    assert.ok(quadro.indisponiveis.some((item) => item.pessoa.nomeExibicao === "Carlos Entregador" && item.estado === "indisponivel"));
    ...
    assert.equal(depois.fila.some((item) => item.pessoa.nomeExibicao === "Carlos Entregador"), false);
    assert.ok(depois.foraDaBase.some((item) => item.pessoa.nomeExibicao === "Carlos Entregador"));
  });
  ...
  it("deixar de aceitar, ou ter o vínculo desativado, remove da fila na hora", async () => {
    ...
    assert.equal((await ctx.api(C, "PATCH", `/entregas/vinculos/${carlos}`, { disponivel: false })).statusCode, 200);
    assert.equal((await painel()).fila.some((item) => item.id === carlos), false);
    ...
    const historico = await ctx.banco.select().from(historicoFilaEntregador).where(eq(historicoFilaEntregador.entregadorId, carlos));
    assert.ok(historico.length >= 2);
    assert.ok(historico.some((linha) => linha.motivoSaida !== null));

    await ctx.api(A, "PATCH", `/empresas/${pizzaria.id}/entregadores/${carlos}`, { status: "ativo" });
    await ctx.api(C, "PATCH", `/entregas/vinculos/${carlos}`, { disponivel: true });
  });
  ...
  it("entradas quase simultâneas mantêm a fila determinística", async () => {
    ...
    await Promise.all([
      enviarLocalizacao(P, paulo, NA_BASE).then(() => enviarLocalizacao(P, paulo, NA_BASE)),
      enviarLocalizacao(C, carlos, NA_BASE).then(() => enviarLocalizacao(C, carlos, NA_BASE)),
      enviarLocalizacao(J, joao, NA_BASE).then(() => enviarLocalizacao(J, joao, NA_BASE)),
    ]);

    const quadro = await painel();
    assert.equal(quadro.fila.length, 3);
    assert.deepEqual(quadro.fila.map((item) => item.posicaoFila), [1, 2, 3], "posições sem ambiguidade");
    // A ordem é estável entre leituras.
    assert.deepEqual((await painel()).fila.map((item) => item.id), quadro.fila.map((item) => item.id));
  });
});
...
describe("ciclo do entregador", () => {
  it("receber saída tira da fila; concluir na base recoloca no final", async () => {
    ...
    for (const status of ["recebido", "confirmado", "em_preparacao"] as const) {
      atual = (await ctx.api(A, "POST", `/empresas/${pizzaria.id}/pedidos/${atual.id}/avancar`, { statusAtual: status })).json();
    }
    const saida = await ctx.api(A, "POST", `/empresas/${pizzaria.id}/saidas`, { entregadorId: paulo, pedidoIds: [pedido.id] });
    assert.equal(saida.statusCode, 201, saida.body);

    // Saiu da fila: Carlos assume a primeira posição.
    assert.deepEqual((await painel()).fila.map((item) => item.id), [carlos]);

    // Conclui a entrega; ele continua na base e aceitando → volta ao FINAL da fila.
    assert.equal((await ctx.api(A, "POST", `/empresas/${pizzaria.id}/saidas/${saida.json().id}/iniciar`)).statusCode, 200);
    for (const status of ["saiu_para_entrega", "em_rota"] as const) {
      assert.equal((await ctx.api(A, "POST", `/empresas/${pizzaria.id}/pedidos/${pedido.id}/avancar`, { statusAtual: status })).statusCode, 200);
    }
    assert.deepEqual((await painel()).fila.map((item) => item.id), [carlos, paulo], "volta no fim, não na frente");
    // Concluir a saída não desligou "aceitando entregas".
    const situacoes: ListaSituacoesOperacionais = (await ctx.api(P, "GET", "/entregas/situacao")).json();
    assert.equal(situacoes.situacoes.find((item) => item.entregadorId === paulo)?.disponivel, true);
  });

  it("o entregador vê a própria posição, sem detalhes dos outros", async () => {
    const situacoes: ListaSituacoesOperacionais = (await ctx.api(P, "GET", "/entregas/situacao")).json();
    const naPizzaria = situacoes.situacoes.find((item) => item.entregadorId === paulo);
  ...
  it("empresa A não enxerga fila, presença nem base da empresa B", async () => {
    const outraDona = (await ctx.api(C, "POST", "/empresas", { nome: "Padaria Rival", nomeUsuario: `${PREFIXO}_rival`, slug: `${PREFIXO}-rival` })).json();

    assert.equal((await ctx.api(C, "GET", `/empresas/${pizzaria.id}/operacao`)).statusCode, 404);
    assert.equal((await ctx.api(C, "GET", `/empresas/${pizzaria.id}/base`)).statusCode, 404);
    assert.equal((await ctx.api(A, "GET", `/empresas/${outraDona.id}/operacao`)).statusCode, 404);
    ...
    assert.equal(daFarmacia.baseConfigurada, false, "sem ponto confirmado não há geofence");
  });
});
...
describe("realtime da operação", () => {
  it("empresa recebe o painel; o entregador recebe só a própria situação", async () => {
    ...
    const painelNoEntregador = coletar<EventoFilaAtualizada>(socketJoao, EVENTO_FILA_ATUALIZADA);

    await chegarNaBase(J, joao);
    await aguardarAte(() => paineis.length >= 1 && doJoao.length >= 1);

    assert.ok(paineis.at(-1)?.painel.fila.some((item) => item.id === joao), "a empresa vê a fila atualizada");
    assert.equal(doJoao.at(-1)?.situacao.entregadorId, joao);
    assert.equal(doJoao.at(-1)?.situacao.naBase, true);
    assert.equal(painelNoEntregador.length, 0, "o entregador não recebe o painel da empresa");
    assert.equal(doPaulo.length, 0, "ninguém recebe a situação de outro entregador");
...
describe("aptidão operacional", () => {
  it("entregador inapto não entra na fila, mesmo aceitando e na base", async () => {
    ...
    await ctx.banco.update(entregadoresEmpresa).set({ aptoParaSaida: false }).where(eq(entregadoresEmpresa.id, joao));
    assert.equal((await ctx.api(J, "PATCH", `/entregas/vinculos/${joao}`, { disponivel: false })).statusCode, 200);
    assert.equal((await ctx.api(J, "PATCH", `/entregas/vinculos/${joao}`, { disponivel: true })).statusCode, 200);

    const quadro = await painel();
    assert.equal(quadro.fila.some((item) => item.id === joao), false, "inapto não entra na fila");
    assert.ok(quadro.foraDaBase.some((item) => item.id === joao && item.estado === "inapto"));

    await ctx.banco.update(entregadoresEmpresa).set({ aptoParaSaida: true }).where(eq(entregadoresEmpresa.id, joao));
  });
});
