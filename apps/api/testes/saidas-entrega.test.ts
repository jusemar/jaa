...
const avancar = (
  pedidoId: string,
  statusAtual: string,
  empresa: Empresa = pizzaria,
) =>
  ctx.api(A, "POST", `/empresas/${empresa.id}/pedidos/${pedidoId}/avancar`, {
    statusAtual,
  });
...
before(async () => {
  ...
  dipirona = (
    await ctx.api(A, "POST", `/empresas/${farmacia.id}/produtos`, {
      nome: "Dipirona",
      precoCentavos: 890,
    })
  ).json();
  ...
  for (const [indice, cliente] of [B1, B2, B3].entries()) {
    conversas.set(
      `${cliente.identidadeId}:${pizzaria.id}`,
      await ctx.abrirConversa(cliente, `${PREFIXO}_pizza`),
    );
    conversas.set(
      `${cliente.identidadeId}:${farmacia.id}`,
      await ctx.abrirConversa(cliente, `${PREFIXO}_farma`),
    );
    deslocamento += 0.01;
    enderecos.set(
      cliente.identidadeId,
      await ctx.criarEnderecoConfirmado(
        cliente,
        { apelido: `Casa ${indice + 1}`, numero: `${100 + indice}` },
        { latitude: -19.91 - deslocamento, longitude: -43.93 - deslocamento },
      ),
    );
  }
  ...
  naFarmacia = await ctx.criarEntregadorAtivo(
    A,
    farmacia.id,
    P,
    `${PREFIXO}_p`,
...
describe("montar a saída", () => {
  it("agrupa vários pedidos prontos, sugere uma sequência e atribui tudo ao entregador", async () => {
    const pedidos = [
      await pedidoPronto(B1),
      await pedidoPronto(B2),
      await pedidoPronto(B3),
    ];

    const resposta = await criarSaida(A, pizzaria, {
      entregadorId: paulo,
    ...
    assert.deepEqual(
      saida.paradas.map((parada) => parada.posicao),
      [1, 2, 3],
    );
    assert.deepEqual(
      saida.paradas.map((parada) => parada.numeroPedido),
      pedidos.map((pedido) => pedido.numero),
    );
    assert.deepEqual(
      [...new Set(saida.paradas.map((parada) => parada.pedidoId))].length,
      3,
    );
    ...
    assert.equal(
      saida.paradas.every(
        (parada) =>
          parada.destino.latitude !== null &&
          parada.destino.logradouro === "Rua das Flores",
      ),
      true,
    );
    ...
    for (const pedido of pedidos)
      assert.equal(
        (await ctx.api(P, "GET", `/entregas/${pedido.id}`)).statusCode,
        200,
      );
  });
  ...
  it("recusa pedido que não está pronto, de outra empresa, inexistente ou já em outra saída", async () => {
    ...
    for (const pedidoIds of [
      [naoPronto.id],
      [naFarmaciaPedido.id],
      [randomUUID()],
      [pronto.id, naoPronto.id],
    ]) {
      const recusa = await criarSaida(A, pizzaria, {
        entregadorId: paulo,
        pedidoIds,
      });
    ...
    assert.equal(
      await ctx.banco
        .select()
        .from(paradasSaida)
        .where(eq(paradasSaida.pedidoId, pronto.id))
    ...
    const repetido = await criarSaida(A, pizzaria, {
      entregadorId: carlos,
      pedidoIds: [pronto.id],
    });
  ...
  it("uma saída é de UMA empresa: nunca mistura pedidos de empresas diferentes", async () => {
    ...
    const misturada = await criarSaida(A, pizzaria, {
      entregadorId: paulo,
      pedidoIds: [daPizzaria.id, daFarmacia.id],
    });
    ...
    assert.equal(
      (
        await criarSaida(A, farmacia, {
          entregadorId: naFarmacia,
          pedidoIds: [daFarmacia.id],
        })
      ).statusCode,
      201,
    );
  ...
  it("dois gestores disputando o mesmo pedido: só uma saída vence", async () => {
    ...
    const paradas = await ctx.banco
      .select()
      .from(paradasSaida)
      .where(eq(paradasSaida.pedidoId, pedido.id));
    assert.equal(
      paradas.filter((parada) => parada.encerradaEm === null).length,
      1,
      "o pedido está em uma única saída ativa",
    );
  });
  ...
  it("empresa diferente e estranhos não acessam a saída", async () => {
    ...
    assert.equal(
      (await ctx.api(A, "GET", `/empresas/${farmacia.id}/saidas/${saida.id}`))
        .statusCode,
      404,
    );
    assert.equal(
      (await ctx.api(B1, "GET", `/empresas/${pizzaria.id}/saidas/${saida.id}`))
        .statusCode,
      404,
    );
    assert.equal(
      (await ctx.api(C, "GET", `/entregas/saidas/${saida.id}`)).statusCode,
      404,
      "entregador que não é o da saída",
    );
    assert.equal(
      (await ctx.api(null, "GET", "/entregas/saidas")).statusCode,
      401,
    );
    assert.equal(
      (await ctx.api(P, "GET", `/entregas/saidas/${saida.id}`)).statusCode,
      200,
    );
  });
});
...
describe("iniciar a saída", () => {
  it("leva os pedidos para 'saiu para entrega' pela máquina de estados, com histórico", async () => {
    ...
    const antesDaLiberacao = await ctx.api(
      P,
      "POST",
      `/entregas/saidas/${saida.id}/iniciar`,
    );
    assert.equal(
      antesDaLiberacao.statusCode,
      409,
    ...
    for (const pedido of pedidos) {
      ...
      assert.deepEqual(
        atual.historico.map((evento) => evento.status),
        ["recebido", "em_preparacao", "pronto", "saiu_para_entrega"],
      );
    }
    ...
    assert.equal(
      (
        await ctx.api(
          A,
          "POST",
          `/empresas/${pizzaria.id}/saidas/${saida.id}/iniciar`,
        )
      ).statusCode,
      404,
    );
  ...
  it("o ENTREGADOR atual inicia a própria saída pela mesma regra — e não inicia de novo", async () => {
    ...
    assert.equal(
      (await ctx.api(C, "POST", `/entregas/saidas/${saida.id}/iniciar`))
        .statusCode,
      404,
    );
    // Cliente, então, muito menos.
    assert.equal(
      (await ctx.api(B1, "POST", `/entregas/saidas/${saida.id}/iniciar`))
        .statusCode,
      404,
    );
    ...
    const posicao = await ctx.api(
      P,
      "POST",
      `/entregas/saidas/${saida.id}/posicao`,
      {
        latitude: -19.92,
        longitude: -43.94,
        precisaoMetros: 10,
    ...
    assert.ok(
      posicao.statusCode >= 200 && posicao.statusCode < 300,
      posicao.body,
    );
  });
});

describe("conclusão pelo entregador", () => {
  const concluir = (pessoa: Pessoa, saidaId: string, pedidoId: string) =>
    ctx.api(
      pessoa,
      "POST",
  ...
  it("só o entregador atribuído conclui e somente depois de iniciar", async () => {
    const pedido = await pedidoPronto(B1);
    const saida: SaidaEntrega = (
      await criarSaida(A, pizzaria, {
        entregadorId: paulo,
    ...
    assert.equal((await concluir(P, saida.id, pedido.id)).statusCode, 409);
    assert.equal((await concluir(C, saida.id, pedido.id)).statusCode, 404);
    assert.equal((await liberarSaida(saida.id)).statusCode, 200);
    assert.equal(
      (await ctx.api(P, "POST", `/entregas/saidas/${saida.id}/iniciar`))
        .statusCode,
      200,
    );
    assert.equal((await concluir(C, saida.id, pedido.id)).statusCode, 404);
  ...
  it("uma parada conclui pedido e rota, bloqueia repetição e preserva indisponibilidade", async () => {
    const pedido = await pedidoPronto(B2);
    const saida: SaidaEntrega = (
      await criarSaida(A, pizzaria, {
        entregadorId: paulo,
    ...
    assert.equal(
      (
        await ctx.api(P, "PATCH", `/entregas/vinculos/${paulo}`, {
          disponivel: false,
        })
      ).statusCode,
      200,
    ...
    const resposta = await concluir(P, saida.id, pedido.id);
    assert.equal(resposta.statusCode, 200, resposta.body);
    assert.equal((resposta.json() as SaidaEntrega).status, "concluida");
    const daEmpresa: Pedido = (
      await ctx.api(A, "GET", `/empresas/${pizzaria.id}/pedidos/${pedido.id}`)
    ).json();
    const doCliente: Pedido = (
      await ctx.api(B2, "GET", `/pedidos/${pedido.id}`)
    ).json();
    assert.equal(daEmpresa.status, "entregue");
    assert.equal(doCliente.status, "entregue");
    assert.equal((await concluir(P, saida.id, pedido.id)).statusCode, 409);

    const vinculos = (await ctx.api(P, "GET", "/entregas/vinculos")).json();
    assert.equal(
      vinculos.vinculos.find((vinculo: { id: string }) => vinculo.id === paulo)
        ?.disponivel,
      false,
    );
    assert.equal(
  ...
  it("em rota com várias paradas conclui apenas a próxima e avança a sequência", async () => {
    const pedidos = [await pedidoPronto(B1), await pedidoPronto(B3)];
    const criada: SaidaEntrega = (
      await criarSaida(A, pizzaria, {
        entregadorId: paulo,
        pedidoIds: pedidos.map((pedido) => pedido.id),
      })
    ).json();
    assert.equal((await liberarSaida(criada.id)).statusCode, 200);
    const iniciada: SaidaEntrega = (
      await ctx.api(P, "POST", `/entregas/saidas/${criada.id}/iniciar`)
    ).json();
    const [primeira, segunda] = iniciada.paradas;

    assert.equal(
      (await concluir(P, iniciada.id, segunda?.pedidoId as string)).statusCode,
      409,
      "não pula a próxima parada",
    );
    const resposta = await concluir(
      P,
      iniciada.id,
      primeira?.pedidoId as string,
    );
    assert.equal(resposta.statusCode, 200, resposta.body);
    const atual: SaidaEntrega = resposta.json();
    assert.equal(atual.status, "em_andamento");
    assert.equal(
      atual.paradas.find((parada) => parada.pedidoId === primeira?.pedidoId)
        ?.statusPedido,
      "entregue",
    );
    assert.equal(
      atual.paradas.find((parada) => parada.encerradaEm === null)?.pedidoId,
      segunda?.pedidoId,
    );
  });
});
...
describe("sequência e reordenação", () => {
  ...
  it("o entregador reordena a própria sequência e a versão avança", async () => {
    ...
    const resposta = await ctx.api(
      P,
      "PATCH",
      `/entregas/saidas/${saida.id}/sequencia`,
      { versaoSequencia: saida.versaoSequencia, pedidoIds: invertida },
    );
    assert.equal(resposta.statusCode, 200, resposta.body);
    const reordenada: SaidaEntrega = resposta.json();

    assert.deepEqual(
      reordenada.paradas.map((parada) => parada.pedidoId),
      invertida,
    );
    assert.deepEqual(
      reordenada.paradas.map((parada) => parada.posicao),
      [1, 2, 3],
    );
    assert.equal(reordenada.versaoSequencia, saida.versaoSequencia + 1);
    // Reordenar preserva TODAS as paradas exatamente uma vez.
    assert.deepEqual(
      [...reordenada.paradas.map((parada) => parada.pedidoId)].sort(),
      [...saida.paradas.map((parada) => parada.pedidoId)].sort(),
    );
  });
  ...
  it("versão antiga não sobrescreve sequência mais nova", async () => {
    ...
    assert.equal(
      (
        await ctx.api(P, "PATCH", `/entregas/saidas/${saida.id}/sequencia`, {
          versaoSequencia: saida.versaoSequencia,
          pedidoIds: [...ordem].reverse(),
        })
      ).statusCode,
      200,
    );

    const atrasada = await ctx.api(
      P,
      "PATCH",
      `/entregas/saidas/${saida.id}/sequencia`,
      { versaoSequencia: saida.versaoSequencia, pedidoIds: ordem },
    );
    assert.equal(atrasada.statusCode, 409);
    assert.equal(atrasada.json().codigo, "SEQUENCIA_DESATUALIZADA");
    const atual: SaidaEntrega = (
      await ctx.api(P, "GET", `/entregas/saidas/${saida.id}`)
    ).json();
    assert.deepEqual(
      atual.paradas.map((parada) => parada.pedidoId),
      [...ordem].reverse(),
      "a ordem mais nova continua valendo",
    );
  });
  ...
  it("só o entregador da saída reordena; a empresa e outros não", async () => {
    ...
    assert.equal(
      (
        await ctx.api(P, "PATCH", `/entregas/saidas/${saida.id}/sequencia`, {
          versaoSequencia: saida.versaoSequencia,
          pedidoIds: [ordem[0], ordem[0], ordem[1]],
        })
      ).statusCode,
      400,
    );
  ...
  it("entrega concluída e cancelamento tiram a parada da sequência sem apagar histórico", async () => {
    const saida = await saidaComTres();
    const [primeiro, segundo, terceiro] = saida.paradas.map(
      (parada) => parada.pedidoId,
    );

    // saiu_para_entrega → entregue, sem a etapa redundante EM_ROTA.
    assert.equal(
      (await avancar(primeiro as string, "saiu_para_entrega")).statusCode,
      200,
    );
    ...
    assert.equal(
      (
        await ctx.api(
          A,
          "POST",
          `/empresas/${pizzaria.id}/pedidos/${segundo}/cancelar`,
          { statusAtual: "saiu_para_entrega", motivo: "Cliente desistiu" },
        )
      ).statusCode,
      200,
    );
    ...
    const atual: SaidaEntrega = (
      await ctx.api(A, "GET", `/empresas/${pizzaria.id}/saidas/${saida.id}`)
    ).json();
    assert.equal(atual.paradas.length, 3, "histórico das paradas permanece");
    const ativas = atual.paradas.filter(
      (parada) => parada.encerradaEm === null,
    );
    assert.deepEqual(
      ativas.map((parada) => parada.pedidoId),
      [terceiro],
    );
    assert.equal(
      atual.paradas.find((parada) => parada.pedidoId === primeiro)
        ?.motivoEncerramento,
      "Pedido entregue",
    );
    assert.equal(atual.status, "em_andamento");
    ...
    assert.equal(
      (await avancar(terceiro as string, "saiu_para_entrega")).statusCode,
      200,
    );
    const concluida: SaidaEntrega = (
      await ctx.api(A, "GET", `/empresas/${pizzaria.id}/saidas/${saida.id}`)
    ).json();
  ...
  it("pedido dentro de saída não é reatribuído individualmente", async () => {
    ...
    const recusa = await ctx.api(
      A,
      "POST",
      `/empresas/${pizzaria.id}/pedidos/${pedidoId}/entrega`,
      { entregadorId: carlos },
    );
    assert.equal(recusa.statusCode, 409);
    assert.equal(recusa.json().codigo, "SAIDA_EM_ANDAMENTO");
    assert.equal(
      (await ctx.api(P, "GET", `/entregas/${pedidoId}`)).statusCode,
      200,
      "continua com quem está na saída",
    );
    assert.equal(
      (await ctx.api(C, "GET", `/entregas/${pedidoId}`)).statusCode,
      404,
    );
  });
  ...
  it("ficar indisponível depois não desfaz a saída já recebida", async () => {
    ...
    const minhas: ListaSaidas = (
      await ctx.api(P, "GET", "/entregas/saidas")
    ).json();
    assert.ok(
      minhas.saidas.some((item) => item.id === saida.id),
      "a saída em andamento continua dele",
    );
    assert.equal(
      (await ctx.api(P, "GET", `/entregas/saidas/${saida.id}`)).statusCode,
      200,
...
describe("fila do cliente", () => {
  it("cada cliente recebe só a própria posição, derivada da sequência", async () => {
    ...
    const antes: FilaDoPedido = (
      await ctx.api(B1, "GET", `/pedidos/${pedidos[0]?.id}/fila`)
    ).json();
    ...
    for (const [posicao, pedidoId] of ordem.entries()) {
      ...
      const fila: FilaDoPedido = (
        await ctx.api(cliente, "GET", `/pedidos/${pedidoId}/fila`)
      ).json();
    ...
    assert.equal(
      (
        await ctx.api(P, "PATCH", `/entregas/saidas/${saida.id}/sequencia`, {
          versaoSequencia: emOrdem.versaoSequencia,
          pedidoIds: invertida,
        })
      ).statusCode,
      200,
    ...
    const fila: FilaDoPedido = (
      await ctx.api(ultimoAgoraPrimeiro, "GET", `/pedidos/${invertida[0]}/fila`)
    ).json();
  ...
  it("o cliente não alcança a saída nem a fila de outro pedido", async () => {
    ...
    assert.equal(
      (await ctx.api(B2, "GET", `/pedidos/${pedido.id}/fila`)).statusCode,
      404,
      "fila de pedido alheio",
    );
    assert.equal(
      (await ctx.api(B1, "GET", `/entregas/saidas/${saida.id}`)).statusCode,
      404,
      "cliente não abre a saída",
    );
    assert.equal(
    ...
    assert.equal(
      (await ctx.api(B1, "GET", `/pedidos/${pedido.id}/fila`)).statusCode,
      200,
      "mas vê a própria fila",
    );
  });
});
...
describe("realtime da saída", () => {
  it("empresa e entregador recebem a saída; cada cliente recebe só a própria fila", async () => {
    const pedidos = [await pedidoPronto(B1), await pedidoPronto(B2)];
    const [socketEmpresa, socketPaulo, socketB1, socketB2, socketCarlos] =
      await Promise.all([
    ...
    const daEmpresa = coletar<EventoSaidaAtualizada>(
      socketEmpresa,
      EVENTO_SAIDA_ATUALIZADA,
    );
    const doPaulo = coletar<EventoSaidaAtualizada>(
      socketPaulo,
      EVENTO_SAIDA_ATUALIZADA,
    );
    const doCarlos = coletar<EventoSaidaAtualizada>(
      socketCarlos,
      EVENTO_SAIDA_ATUALIZADA,
    );
    const saidaNoCliente = coletar<EventoSaidaAtualizada>(
      socketB1,
      EVENTO_SAIDA_ATUALIZADA,
    );
    ...
    assert.equal(
      saidaNoCliente.length,
      0,
      "o cliente NUNCA recebe a saída completa",
    );
    // O cliente recebe só a própria posição.
    assert.equal(filaB1[0]?.pedidoId, pedidos[0]?.id);
    assert.deepEqual(Object.keys(filaB1[0] ?? {}).sort(), [
      "entregasAntes",
      "pedidoId",
      "situacao",
    ]);
    ...
    assert.equal(
      (
        await ctx.api(P, "PATCH", `/entregas/saidas/${saida.id}/sequencia`, {
          versaoSequencia: emOrdem.versaoSequencia,
          pedidoIds: [...emOrdem.paradas.map((p) => p.pedidoId)].reverse(),
        })
      ).statusCode,
      200,
    );
    await aguardarAte(
      () => filaB1.length > antesDaTroca && daEmpresa.length >= 2,
    );
...
describe("saída no banco", () => {
  it("guarda quem montou, quando, o entregador e a sequência inicial", async () => {
    ...
    const [registro] = await ctx.banco
      .select()
      .from(saidasEntrega)
    ...
    assert.equal(
      typeof registro?.criadaPorUsuarioId,
      "string",
      "auditoria interna de quem montou",
    );
    assert.equal(registro?.versaoSequencia, 1);
    // A auditoria nunca é serializada para ninguém.
    assert.equal(
      JSON.stringify(saida).includes(
        registro?.criadaPorUsuarioId ?? "impossível",
      ),
      false,
    );
  });
