...
const ctx = criarAmbienteIntegracao({ telefones: TELEFONES, prefixoIp: "198.51.105." });

let joao: Pessoa;
let semSenha: Pessoa;

const entrar = (identificador: string, senha: string) => ctx.api(null, "POST", "/autenticacao/entrar", { identificador, senha });
...
describe("definir a senha depois do cadastro por OTP", () => {
  ...
  it("senha curta demais é recusada antes de chegar ao banco", async () => {
    ...
    assert.equal((await ctx.api(joao, "GET", "/conta/senha")).json().definida, false);
  });

  it("define a primeira senha", async () => {
    const definida = await ctx.api(joao, "POST", "/conta/senha", { senha: SENHA });
    assert.equal(definida.statusCode, 200, definida.body);
    assert.equal((await ctx.api(joao, "GET", "/conta/senha")).json().definida, true);
  });

  it("trocar a senha EXIGE a senha atual: sessão aberta não basta", async () => {
    const semAtual = await ctx.api(joao, "POST", "/conta/senha", { senha: "outra-senha-qualquer" });
    ...
    const errada = await ctx.api(joao, "POST", "/conta/senha", { senha: "outra-senha-qualquer", senhaAtual: "nao-e-essa" });
    assert.notEqual(errada.statusCode, 200);
  });

  it("sem sessão não se define senha de ninguém", async () => {
    assert.equal((await ctx.api(null, "POST", "/conta/senha", { senha: SENHA })).statusCode, 401);
...
describe("entrar com telefone ou @usuario", () => {
  it("entra pelo @usuario e cria sessão de verdade", async () => {
    ...
    assert.ok(cookie?.value, "a sessão é o cookie do Better Auth, não um token próprio");

    // A sessão vale de verdade nas rotas do Jaa.
    const conta = await ctx.api({ ...joao, cookie: `${cookie.name}=${cookie.value}` }, "GET", "/perfil");
  ...
  it("entra pelo celular, com ou sem máscara, com ou sem @ no identificador", async () => {
    ...
    assert.equal((await entrar("@joao_senha", SENHA)).statusCode, 200);
    assert.equal((await entrar("JOAO_SENHA", SENHA)).statusCode, 200, "@usuario é canônico em minúsculas");
  });
  ...
  it("senha errada, @usuario inexistente e conta sem senha respondem IGUAL", async () => {
    ...
    for (const resposta of [senhaErrada, inexistente, contaSemSenha]) {
      ...
      assert.equal(resposta.cookies.some((c) => c.name === "better-auth.session_token" && c.value), false);
    }
  ...
  it("quem nunca definiu senha continua entrando por OTP", async () => {
    ...
    assert.equal((await ctx.api(semSenha, "GET", "/conta/senha")).json().definida, false);
  });

  it("identificador vazio é erro de dados, não tentativa de login", async () => {
    const vazio = await ctx.api(null, "POST", "/autenticacao/entrar", { identificador: "   ", senha: SENHA });
  ...
  it("o @usuario de uma EMPRESA não é uma conta e não entra", async () => {
    const empresa = await ctx.api(joao, "POST", "/empresas", { nome: "Loja do João", nomeUsuario: "loja_do_joao", slug: "loja-do-joao" });
