...
const parametrosSaidaDaEmpresaSchema = z.object({ empresaId: z.uuid(), saidaId: z.uuid() });
const parametrosZonaSchema = z.object({ empresaId: z.uuid(), zonaId: z.uuid() });
...
const SAIDA_NAO_ENCONTRADA: ErroApi = { codigo: "SAIDA_NAO_ENCONTRADA", mensagem: "Saída de entrega não encontrada." };
const ZONA_NAO_ENCONTRADA: ErroApi = { codigo: "ZONA_NAO_ENCONTRADA", mensagem: "Zona de entrega não encontrada." };
...
export function registrarRotasEntregas(
  ...
  servidor.post("/empresas/:empresaId/pedidos/:pedidoId/entrega", { preHandler }, async (requisicao, resposta) => {
    ...
    switch (resultado.tipo) {
      ...
      case "pedido-em-saida":
        return responder(resposta, 409, { codigo: "SAIDA_EM_ANDAMENTO", mensagem: "Este pedido faz parte de uma saída de entrega; troque pela saída, não por pedido." });
      case "conflito":
  ...
  servidor.get("/empresas/:empresaId/base", { preHandler }, async (requisicao, resposta) => {
    const { usuarioId } = obterIdentidadeExigida(requisicao);
    const parametros = parametrosEmpresaSchema.safeParse(requisicao.params);
    if (!parametros.success) return responder(resposta, 400, { codigo: "DADOS_INVALIDOS", mensagem: "Empresa inválida." });
    ...
    if (!resultado.base) return responder(resposta, 404, { codigo: "BASE_NAO_CONFIGURADA", mensagem: "Esta empresa ainda não configurou a base de onde saem as entregas." });
    const base: BaseEmpresa = serializarBase(resultado.base);
    return base;
  });

  servidor.post("/empresas/:empresaId/base", { preHandler }, async (requisicao, resposta) => {
    const { usuarioId } = obterIdentidadeExigida(requisicao);
    const parametros = parametrosEmpresaSchema.safeParse(requisicao.params);
    const entrada = salvarBaseEntradaSchema.safeParse(requisicao.body);
    if (!parametros.success) return responder(resposta, 400, { codigo: "DADOS_INVALIDOS", mensagem: "Empresa inválida." });
    if (!entrada.success) return responder(resposta, 400, { codigo: "DADOS_INVALIDOS", mensagem: entrada.error.issues[0]?.message ?? "Base inválida." });

    const resultado = await salvarBaseAutorizada(banco, usuarioId, parametros.data.empresaId, entrada.data);
    if (resultado.tipo !== "base") return responder(resposta, 404, EMPRESA_NAO_ENCONTRADA);
    return serializarBase(resultado.base);
  });

  // Confirmação EXPLÍCITA do ponto da base (o mapa nunca corrige o endereço digitado).
  servidor.post("/empresas/:empresaId/base/localizacao", { preHandler }, async (requisicao, resposta) => {
    const { usuarioId } = obterIdentidadeExigida(requisicao);
    const parametros = parametrosEmpresaSchema.safeParse(requisicao.params);
    const entrada = confirmarPontoBaseEntradaSchema.safeParse(requisicao.body);
    if (!parametros.success) return responder(resposta, 400, { codigo: "DADOS_INVALIDOS", mensagem: "Empresa inválida." });
    if (!entrada.success) return responder(resposta, 400, { codigo: "DADOS_INVALIDOS", mensagem: "Coordenadas inválidas." });

    const resultado = await confirmarPontoBaseAutorizado(banco, usuarioId, parametros.data.empresaId, entrada.data);
    if (resultado.tipo === "empresa-nao-encontrada") return responder(resposta, 404, EMPRESA_NAO_ENCONTRADA);
  ...
  servidor.get("/empresas/:empresaId/operacao", { preHandler }, async (requisicao, resposta) => {
    const { usuarioId } = obterIdentidadeExigida(requisicao);
    const parametros = parametrosEmpresaSchema.safeParse(requisicao.params);
    if (!parametros.success) return responder(resposta, 400, { codigo: "DADOS_INVALIDOS", mensagem: "Empresa inválida." });

    const resultado = await obterPainelAutorizado(banco, usuarioId, parametros.data.empresaId);
    if (resultado.tipo !== "painel") return responder(resposta, 404, EMPRESA_NAO_ENCONTRADA);
  ...
  servidor.post("/entregas/vinculos/:entregadorId/localizacao", { preHandler }, async (requisicao, resposta) => {
    const { usuarioId } = obterIdentidadeExigida(requisicao);
    ...
    const resultado = await processarLocalizacao(banco, usuarioId, parametros.data.entregadorId, {
    ...
    switch (resultado.tipo) {
      ...
      case "base-nao-configurada":
        return responder(resposta, 409, { codigo: "BASE_NAO_CONFIGURADA", mensagem: "Esta empresa ainda não confirmou o ponto da base." });
      case "leitura-imprecisa":
        return responder(resposta, 409, { codigo: "LOCALIZACAO_IMPRECISA", mensagem: "Leitura de localização antiga ou imprecisa demais para confirmar presença." });
      case "processada": {
        // Entrou/saiu da base ou da fila: empresa e entregador acompanham sem F5.
        if (resultado.presencaMudou || resultado.filaMudou) await publicarOperacao(dependencias, resultado.registro);
        // Entrou na fila da base: se havia saída esperando, ela é despachada agora (sem F5).
        if (resultado.filaMudou) await aoEntregadorEntrarNaFila(dependencias, resultado.registro);
  ...
  servidor.get("/entregas/situacao", { preHandler }, async (requisicao) => {
    const { usuarioId } = obterIdentidadeExigida(requisicao);
    const lista: ListaSituacoesOperacionais = { situacoes: await listarMinhasSituacoes(banco, usuarioId) };
    return lista;
  });
  ...
  servidor.get("/empresas/:empresaId/zonas", { preHandler }, async (requisicao, resposta) => {
    const { usuarioId } = obterIdentidadeExigida(requisicao);
    const parametros = parametrosEmpresaSchema.safeParse(requisicao.params);
    if (!parametros.success) return responder(resposta, 400, { codigo: "DADOS_INVALIDOS", mensagem: "Empresa inválida." });

    const resultado = await listarZonasAutorizado(banco, usuarioId, parametros.data.empresaId);
    if (resultado.tipo !== "lista") return responder(resposta, 404, EMPRESA_NAO_ENCONTRADA);
    const lista: ListaZonas = { zonas: resultado.zonas };
    return lista;
  });

  const responderZona = async (resposta: FastifyReply, resultado: Awaited<ReturnType<typeof criarZonaAutorizada>>, empresaId: string, criada: boolean) => {
    switch (resultado.tipo) {
      case "empresa-nao-encontrada":
        return responder(resposta, 404, EMPRESA_NAO_ENCONTRADA);
      ...
      case "geometria-invalida":
        return responder(resposta, 400, { codigo: "ZONA_INVALIDA", mensagem: "O contorno da zona precisa ser um polígono simples, sem cruzar as próprias linhas." });
      case "nome-duplicado":
        return responder(resposta, 409, { codigo: "ZONA_INVALIDA", mensagem: "Já existe uma zona com esse nome nesta empresa." });
      case "sobreposta":
  ...
  const listarZonasAutorizadoOuVazio = async (empresaId: string) => (await montarPainelDespacho(banco, empresaId)).zonas;

  servidor.post("/empresas/:empresaId/zonas", { preHandler }, async (requisicao, resposta) => {
    const { usuarioId } = obterIdentidadeExigida(requisicao);
    const parametros = parametrosEmpresaSchema.safeParse(requisicao.params);
    const entrada = salvarZonaEntradaSchema.safeParse(requisicao.body);
    if (!parametros.success) return responder(resposta, 400, { codigo: "DADOS_INVALIDOS", mensagem: "Empresa inválida." });
    if (!entrada.success) return responder(resposta, 400, { codigo: "ZONA_INVALIDA", mensagem: entrada.error.issues[0]?.message ?? "Zona inválida." });

    const resultado = await criarZonaAutorizada(banco, usuarioId, parametros.data.empresaId, entrada.data);
    return responderZona(resposta, resultado, parametros.data.empresaId, true);
  });

  servidor.patch("/empresas/:empresaId/zonas/:zonaId", { preHandler }, async (requisicao, resposta) => {
    const { usuarioId } = obterIdentidadeExigida(requisicao);
    const parametros = parametrosZonaSchema.safeParse(requisicao.params);
    const entrada = salvarZonaEntradaSchema.safeParse(requisicao.body);
    if (!parametros.success) return responder(resposta, 400, { codigo: "DADOS_INVALIDOS", mensagem: "Zona inválida." });
    if (!entrada.success) return responder(resposta, 400, { codigo: "ZONA_INVALIDA", mensagem: entrada.error.issues[0]?.message ?? "Zona inválida." });

    const resultado = await atualizarZonaAutorizada(banco, usuarioId, parametros.data.empresaId, parametros.data.zonaId, entrada.data);
    return responderZona(resposta, resultado, parametros.data.empresaId, false);
  });

  // Compatibilidade entre zonas: decisão explícita do gestor, normalizada para valer nos dois sentidos.
  servidor.post("/empresas/:empresaId/zonas/:zonaId/compatibilidades", { preHandler }, async (requisicao, resposta) => {
    const { usuarioId } = obterIdentidadeExigida(requisicao);
    const parametros = parametrosZonaSchema.safeParse(requisicao.params);
    const entrada = definirCompatibilidadesEntradaSchema.safeParse(requisicao.body);
    if (!parametros.success) return responder(resposta, 400, { codigo: "DADOS_INVALIDOS", mensagem: "Zona inválida." });
    if (!entrada.success) return responder(resposta, 400, { codigo: "DADOS_INVALIDOS", mensagem: "Informe as zonas compatíveis." });

    const resultado = await definirCompatibilidadesAutorizado(banco, usuarioId, parametros.data.empresaId, parametros.data.zonaId, entrada.data.zonaIds);
    if (resultado.tipo === "empresa-nao-encontrada") return responder(resposta, 404, EMPRESA_NAO_ENCONTRADA);
    if (resultado.tipo !== "definida") return responder(resposta, 404, ZONA_NAO_ENCONTRADA);
    const lista: ListaZonas = { zonas: resultado.zonas };
    return lista;
  });

  // Painel de logística: configuração, zonas e os pedidos que ficaram fora das zonas (pendência).
  servidor.get("/empresas/:empresaId/despacho", { preHandler }, async (requisicao, resposta) => {
    const { usuarioId } = obterIdentidadeExigida(requisicao);
    const parametros = parametrosEmpresaSchema.safeParse(requisicao.params);
    if (!parametros.success) return responder(resposta, 400, { codigo: "DADOS_INVALIDOS", mensagem: "Empresa inválida." });

    const resultado = await obterConfiguracaoAutorizada(banco, usuarioId, parametros.data.empresaId);
    if (resultado.tipo !== "configuracao") return responder(resposta, 404, EMPRESA_NAO_ENCONTRADA);
    ...
    const painel: PainelDespacho = await montarPainelDespacho(banco, parametros.data.empresaId);
    return painel;
  });

  servidor.post("/empresas/:empresaId/despacho", { preHandler }, async (requisicao, resposta) => {
    const { usuarioId } = obterIdentidadeExigida(requisicao);
    const parametros = parametrosEmpresaSchema.safeParse(requisicao.params);
    const entrada = salvarConfiguracaoDespachoEntradaSchema.safeParse(requisicao.body);
    if (!parametros.success) return responder(resposta, 400, { codigo: "DADOS_INVALIDOS", mensagem: "Empresa inválida." });
    if (!entrada.success) return responder(resposta, 400, { codigo: "DADOS_INVALIDOS", mensagem: entrada.error.issues[0]?.message ?? "Configuração inválida." });

    const resultado = await salvarConfiguracaoAutorizada(banco, usuarioId, parametros.data.empresaId, entrada.data);
    if (resultado.tipo !== "configuracao") return responder(resposta, 404, EMPRESA_NAO_ENCONTRADA);
  ...
  servidor.get("/empresas/:empresaId/saidas", { preHandler }, async (requisicao, resposta) => {
    const { usuarioId } = obterIdentidadeExigida(requisicao);
    const parametros = parametrosEmpresaSchema.safeParse(requisicao.params);
    if (!parametros.success) return responder(resposta, 400, { codigo: "DADOS_INVALIDOS", mensagem: "Empresa inválida." });

    const consulta = requisicao.query as { ativas?: string };
    const resultado = await listarSaidasAutorizado(banco, usuarioId, parametros.data.empresaId, consulta.ativas !== "false");
    if (resultado.tipo !== "lista") return responder(resposta, 404, EMPRESA_NAO_ENCONTRADA);
    const lista: ListaSaidas = { saidas: await Promise.all(resultado.saidas.map((saida) => serializarSaidaComEmpresa(banco, saida))) };
    return lista;
  });

  servidor.post("/empresas/:empresaId/saidas", { preHandler }, async (requisicao, resposta) => {
    const { usuarioId } = obterIdentidadeExigida(requisicao);
    const parametros = parametrosEmpresaSchema.safeParse(requisicao.params);
    const entrada = criarSaidaEntradaSchema.safeParse(requisicao.body);
    if (!parametros.success) return responder(resposta, 400, { codigo: "DADOS_INVALIDOS", mensagem: "Empresa inválida." });
    if (!entrada.success) return responder(resposta, 400, { codigo: "DADOS_INVALIDOS", mensagem: "Escolha o entregador e ao menos um pedido." });

    const resultado = await criarSaidaAutorizada(dependencias, usuarioId, parametros.data.empresaId, entrada.data);
    switch (resultado.tipo) {
      case "empresa-nao-encontrada":
        return responder(resposta, 404, EMPRESA_NAO_ENCONTRADA);
      case "entregador-nao-encontrado":
      ...
      case "pedidos-invalidos":
        return responder(resposta, 409, { codigo: "PEDIDOS_INVALIDOS_PARA_SAIDA", mensagem: "Só entram pedidos prontos desta empresa, com destino confirmado e fora de outra saída." });
      case "conflito":
        return responder(resposta, 409, { codigo: "PEDIDOS_INVALIDOS_PARA_SAIDA", mensagem: "Algum pedido entrou em outra saída agora há pouco. Recarregue a lista." });
      case "criada": {
        // Recebeu saída: sai da fila da base (está indo para a rua).
        const aposSaida = await reavaliarFila(banco, entrada.data.entregadorId, "Recebeu saída de entrega");
        if (aposSaida) await publicarOperacao(dependencias, aposSaida.registro);
        await publicarSaida(dependencias, resultado.saida);
        return resposta.code(201).send(await serializarSaidaComEmpresa(banco, resultado.saida));
      }
    }
  });

  servidor.get("/empresas/:empresaId/saidas/:saidaId", { preHandler }, async (requisicao, resposta) => {
    const { usuarioId } = obterIdentidadeExigida(requisicao);
    ...
    const resultado = await obterSaidaDaEmpresaAutorizado(banco, usuarioId, parametros.data.empresaId, parametros.data.saidaId);
    if (resultado.tipo === "empresa-nao-encontrada") return responder(resposta, 404, EMPRESA_NAO_ENCONTRADA);
    if (resultado.tipo !== "saida") return responder(resposta, 404, SAIDA_NAO_ENCONTRADA);
    return serializarSaidaComEmpresa(banco, resultado.saida);
  });

  // Iniciar: o entregador saiu com os pedidos; cada um avança para "saiu para entrega".
  servidor.post("/empresas/:empresaId/saidas/:saidaId/iniciar", { preHandler }, async (requisicao, resposta) => {
    const { usuarioId } = obterIdentidadeExigida(requisicao);
    const parametros = parametrosSaidaDaEmpresaSchema.safeParse(requisicao.params);
    if (!parametros.success) return responder(resposta, 400, { codigo: "DADOS_INVALIDOS", mensagem: "Saída inválida." });

    const { empresaId, saidaId } = parametros.data;
    const resultado = await iniciarSaidaAutorizada(banco, usuarioId, empresaId, saidaId, (pedidoId) =>
      dependencias.avancarPedidoParaEntrega(usuarioId, empresaId, pedidoId),
    );
    if (resultado.tipo === "empresa-nao-encontrada") return responder(resposta, 404, EMPRESA_NAO_ENCONTRADA);
  ...
  /**
   * Fechamento ANTECIPADO pelo gestor: a saída em formação para de receber pedidos e entra na fila de
   * despacho. Automação é padrão, não prisão — mas o gestor não "fura" capacidade nem saída iniciada.
   */
  servidor.post("/empresas/:empresaId/saidas/:saidaId/fechar", { preHandler }, async (requisicao, resposta) => {
    const { usuarioId } = obterIdentidadeExigida(requisicao);
    const parametros = parametrosSaidaDaEmpresaSchema.safeParse(requisicao.params);
    if (!parametros.success) return responder(resposta, 400, { codigo: "DADOS_INVALIDOS", mensagem: "Saída inválida." });
    ...
    if (!(await fecharFormacaoManualmente(dependencias, empresaId, saidaId))) {
      const existente = await obterSaidaDaEmpresaAutorizado(banco, usuarioId, empresaId, saidaId);
      if (existente.tipo !== "saida") return responder(resposta, 404, SAIDA_NAO_ENCONTRADA);
      return responder(resposta, 409, { codigo: "SAIDA_NAO_ESTA_EM_FORMACAO", mensagem: "Esta saída já foi fechada — pedido novo entra na próxima." });
    }
    const atual = await obterSaidaDaEmpresaAutorizado(banco, usuarioId, empresaId, saidaId);
    if (atual.tipo !== "saida") return responder(resposta, 404, SAIDA_NAO_ENCONTRADA);
  ...
  servidor.get("/entregas/saidas", { preHandler }, async (requisicao) => {
    const { usuarioId } = obterIdentidadeExigida(requisicao);
    const consulta = requisicao.query as { ativas?: string };
    const saidas = await listarMinhasSaidas(banco, usuarioId, consulta.ativas !== "false");
    const lista: ListaSaidas = { saidas: await Promise.all(saidas.map((saida) => serializarSaidaComEmpresa(banco, saida))) };
    return lista;
  });

  servidor.get("/entregas/saidas/:saidaId", { preHandler }, async (requisicao, resposta) => {
    const { usuarioId } = obterIdentidadeExigida(requisicao);
    const parametros = parametrosSaidaSchema.safeParse(requisicao.params);
    if (!parametros.success) return responder(resposta, 400, { codigo: "DADOS_INVALIDOS", mensagem: "Saída inválida." });
   ...
   */
  servidor.patch("/entregas/saidas/:saidaId/sequencia", { preHandler }, async (requisicao, resposta) => {
    const { usuarioId } = obterIdentidadeExigida(requisicao);
    const parametros = parametrosSaidaSchema.safeParse(requisicao.params);
    const entrada = reordenarSequenciaEntradaSchema.safeParse(requisicao.body);
    if (!parametros.success) return responder(resposta, 400, { codigo: "DADOS_INVALIDOS", mensagem: "Saída inválida." });
    if (!entrada.success) return responder(resposta, 400, { codigo: "DADOS_INVALIDOS", mensagem: "Informe a nova ordem e a versão da sequência." });

    const resultado = await reordenarMinhaSaida(dependencias, usuarioId, parametros.data.saidaId, entrada.data);
    switch (resultado.tipo) {
      case "saida-nao-encontrada":
        return responder(resposta, 404, SAIDA_NAO_ENCONTRADA);
      case "sequencia-invalida":
        return responder(resposta, 400, { codigo: "DADOS_INVALIDOS", mensagem: "A nova ordem precisa conter exatamente as entregas ativas desta saída." });
      case "versao-desatualizada":
        return responder(resposta, 409, { codigo: "SEQUENCIA_DESATUALIZADA", mensagem: "A sequência mudou enquanto você organizava. Recarregue para ver a ordem atual." });
      case "reordenada":
        await publicarSaida(dependencias, resultado.saida);
        return serializarSaidaComEmpresa(banco, resultado.saida);
    }
  ...
  servidor.patch("/entregas/vinculos/:entregadorId", { preHandler }, async (requisicao, resposta) => {
    ...
    const naFila = await reavaliarFila(banco, parametros.data.entregadorId, "Parou de aceitar entregas");
    if (naFila) {
