...
export function listarEnderecos(banco: Banco, identidadeId: string): Promise<EnderecoRegistro[]> {
  return banco
    .select()
    .from(enderecosCliente)
    .where(and(eq(enderecosCliente.identidadeId, identidadeId), isNull(enderecosCliente.arquivadoEm)))
    .orderBy(desc(enderecosCliente.id));
}

export async function buscarEndereco(banco: Banco, identidadeId: string, enderecoId: string): Promise<EnderecoRegistro | null> {
  const [endereco] = await banco
    .select()
    .from(enderecosCliente)
    .where(and(eq(enderecosCliente.id, enderecoId), eq(enderecosCliente.identidadeId, identidadeId), isNull(enderecosCliente.arquivadoEm)))
    .limit(1);
  return endereco ?? null;
}

export async function contarEnderecos(banco: Banco, identidadeId: string): Promise<number> {
...
export async function inserirEndereco(banco: Banco, identidadeId: string, dados: DadosEndereco): Promise<EnderecoRegistro> {
  // Endereço novo nasce SEM ponto confirmado: a coordenada só existe depois do mapa.
  const [endereco] = await banco.insert(enderecosCliente).values({ ...dados, identidadeId }).returning();
...
export async function confirmarLocalizacao(banco: Banco, identidadeId: string, enderecoId: string, coordenadas: Coordenadas): Promise<EnderecoRegistro | null> {
  const [endereco] = await banco
    .update(enderecosCliente)
...
export async function arquivarEndereco(banco: Banco, identidadeId: string, enderecoId: string): Promise<boolean> {
