...
const ALTURA_TOQUE = "min-h-11"; // 44px: mínimo confortável para o dedo.

type Aparencia = "principal" | "secundario" | "discreto" | "perigo";

const APARENCIAS: Record<Aparencia, string> = {
  principal: "bg-marca text-marca-conteudo hover:opacity-90",
  secundario: "border border-borda bg-superficie text-conteudo hover:bg-superficie-suave",
  discreto: "text-conteudo-suave hover:bg-superficie-suave",
...
export function Botao({
  ...
  return (
    <button
      ...
      // Pílula: é a forma dos botões na referência de UI/UX aprovada, e combina com os balões.
      className={`${ALTURA_TOQUE} inline-flex items-center justify-center gap-2 rounded-full px-4 text-sm font-medium transition-opacity disabled:cursor-not-allowed disabled:opacity-50 ${APARENCIAS[aparencia]} ${larguraTotal ? "w-full" : ""} ${className}`}
    >
      {children}
    </button>
  );
...
const ESTILO_CAMPO = `${ALTURA_TOQUE} w-full rounded-jaa border border-borda bg-superficie px-3 text-base text-conteudo placeholder:text-conteudo-suave/60 disabled:opacity-60`;
...
export function CampoTexto({
  ...
  id,
  ...props
}: InputHTMLAttributes<HTMLInputElement> & { rotulo: string; dica?: string; erro?: string | null; id: string }) {
  return (
...
export function Cartao({ children, className = "" }: { children: ReactNode; className?: string }) {
  return <div className={`rounded-jaa border border-borda bg-superficie shadow-suave ${className}`}>{children}</div>;
}
    ...
    <section aria-label={titulo} className="flex flex-col gap-3">
      <div className="flex flex-wrap items-end justify-between gap-2">
        <div className="flex min-w-0 flex-col gap-0.5">
          <h2 className="fonte-display text-lg font-semibold text-conteudo">{titulo}</h2>
...
export function EstadoVazio({ titulo, descricao, acao }: { titulo: string; descricao: string; acao?: ReactNode }) {
  return (
    <div className="flex flex-col items-center gap-2 rounded-jaa border border-dashed border-borda px-4 py-8 text-center">
      <p className="text-sm font-medium text-conteudo">{titulo}</p>
