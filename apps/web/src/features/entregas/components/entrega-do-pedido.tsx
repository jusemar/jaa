...
export function EntregaDoPedidoEmpresa({
  ...
  return (
    <div data-entrega-pedido className="flex flex-col gap-1 rounded border border-zinc-200 p-2 text-xs">
      ...
      <p data-entregador-atual={atual?.id ?? ""}>
        ...
        {atual && atual.status !== "ativo" && <span className="text-amber-700"> — {ROTULO_STATUS_ENTREGADOR[atual.status]}</span>}
      </p>

      {podeAtribuir && !escolhendo && (
        <button type="button" data-atribuir-entregador onClick={() => setEscolhendo(true)} className="self-start rounded border px-2 py-1">
          {atual ? "Trocar entregador" : "Atribuir entregador"}
        </button>
      )}

      {escolhendo && (
        <div role="group" aria-label="Escolher entregador" className="flex flex-col gap-1 rounded border border-zinc-300 p-2">
          {entregadoresAtivos.length === 0 ? (
            <p className="text-zinc-600">Nenhum entregador ativo. Convide alguém em &quot;Entregadores&quot;.</p>
          ) : (
          ...
          <span className="flex gap-2">
            ...
            <button type="button" onClick={() => setEscolhendo(false)} className="rounded border px-2 py-1">
              Cancelar
            </button>
          </span>
        </div>
      ...
      {entrega && entrega.historico.length > 0 && (
        <ol aria-label="Histórico de entregadores" className="flex flex-col gap-0.5 text-zinc-600">
