...
import { criarSaida, fecharSaida, iniciarSaida, listarEntregadores, listarSaidasDaEmpresa } from "../lib/api-entregas";
...
export function AreaSaidasEmpresa({ empresaId, nomeEmpresa }: { empresaId: string; nomeEmpresa: string }) {
  ...
  return (
    <section aria-label="Saídas de entrega" className="flex flex-col gap-3 rounded border border-zinc-200 p-3">
      ...
      <div className="flex flex-col gap-2 rounded border border-zinc-200 p-2 text-sm">
        <p className="text-xs text-zinc-500">Selecione os pedidos prontos que saem juntos e quem vai levar.</p>
        <ol aria-label="Pedidos prontos" className="flex flex-col gap-1">
          {prontos.length === 0 && <li className="text-xs text-zinc-500">Nenhum pedido pronto agora.</li>}
          {prontos.map((pedido) => (
            <li key={pedido.id}>
        ...
        <label className="flex flex-col gap-1 text-xs">
          Entregador
          <select name="entregadorDaSaida" value={entregadorId} onChange={(evento) => setEntregadorId(evento.target.value)} className="rounded border border-zinc-300 px-2 py-1 text-sm">
        ...
        <button
          ...
          onClick={() => void montar()}
          className="self-start rounded bg-black px-3 py-1.5 text-xs text-white disabled:opacity-50"
        >
      ...
      <ol aria-label="Saídas" className="flex flex-col gap-3">
        {saidas.length === 0 && <li className="text-sm text-zinc-500">Nenhuma saída em aberto.</li>}
        {saidas.map((saida) => (
          <li key={saida.id} data-saida={saida.id} className="flex flex-col gap-2 rounded border border-zinc-200 p-2">
            <div className="flex flex-wrap items-center justify-between gap-2">
              <span className="flex flex-col text-sm font-medium">
                ...
                {saida.zonaPrincipal && (
                  <span data-zona-saida={saida.zonaPrincipal.id} className="text-xs font-normal text-zinc-600">
              ...
              <span className="flex gap-2">
                {saida.status === "em_formacao" && (
                  <button type="button" data-fechar-saida disabled={ocupado} onClick={() => void fechar(saida)} className="rounded border px-2 py-1 text-xs disabled:opacity-50">
                    Fechar agora
                  </button>
                )}
                {saida.status === "preparada" && (
                  <button type="button" data-iniciar-saida disabled={ocupado} onClick={() => void iniciar(saida)} className="rounded bg-black px-2 py-1 text-xs text-white disabled:opacity-50">
                    Iniciar saída
                  </button>
                )}
              </span>
