...
export function AvisoLocalizacao({ permissao }: { permissao: PermissaoLocalizacao }) {
 ...
 */
export function MinhaSituacaoNaBase({ situacoes, permissao, aoPermitir }: { situacoes: SituacaoOperacional[]; permissao: PermissaoLocalizacao; aoPermitir: () => void }) {
 ...
 */
export function usePresencaNaBase(situacoes: SituacaoOperacional[], aoReceberSituacao: (situacao: SituacaoOperacional) => void) {
  ...
  useEffect(() => {
    if (idsAceitando === "" || permissao === "negada" || permissao === "indisponivel") return;
