...
export function AvisosNotificacao({
  ...
  return (
    <div
      ...
      aria-live="polite"
      className="fixed bottom-20 right-4 z-20 flex w-80 max-w-[calc(100vw-2rem)] flex-col gap-2 md:bottom-4"
    >
      {avisos.map((aviso) => (
        <div
          key={aviso.mensagemId}
          data-notificacao={aviso.conversaId}
          role="status"
          className="flex items-start gap-2 rounded-jaa border border-borda bg-superficie p-3 text-sm shadow-suave"
        >
          <button
            type="button"
          ...
          >
            <span className="block truncate font-semibold">
              {aviso.remetente.nomeExibicao}
            </span>
            <span className="line-clamp-2 text-conteudo-suave [overflow-wrap:anywhere]">
              {textoDaPrevia(aviso)}
            </span>
          </button>
          <button
