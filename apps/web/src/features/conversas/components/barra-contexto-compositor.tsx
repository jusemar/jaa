...
export function BarraContextoCompositor({ titulo, texto, aoCancelar, rotuloCancelar }: { titulo: string; texto: string; aoCancelar: () => void; rotuloCancelar: string }) {
  return (
    <div aria-label={titulo} className="flex items-start gap-2 rounded-lg bg-superficie-suave p-1.5">
      <div className="min-w-0 flex-1 rounded border-l-4 border-ouro/60 bg-marca/5 px-2 py-1">
        ...
        <p className="line-clamp-2 whitespace-pre-wrap text-xs text-conteudo-suave [overflow-wrap:anywhere]">{texto}</p>
      </div>
      <button
        type="button"
