...
export function FormularioEndereco({
  ...
  const alterar = (campo: keyof DadosFormularioEndereco) => (valor: string) =>
  ...
  const perderaConfirmacao =
    ...
    alteracaoInvalidaLocalizacao(endereco, {
      ...dados,
  ...
  return (
    <form
      aria-label={endereco ? "Editar endereço" : "Novo endereço"}
      onSubmit={enviar}
    ...
    >
      ...
      {MENSAGEM_CEP[situacaoCep] && (
        <p
          data-situacao-cep={situacaoCep}
          className="text-xs text-conteudo-suave sm:col-span-4"
        >
          {MENSAGEM_CEP[situacaoCep]}
        </p>
      )}
      <Campo
        ...
        maximo={LOGRADOURO_TAMANHO_MAXIMO}
        aoMudar={alterar("logradouro")}
      ...
      <Campo
        ...
        nome="numero"
        valor={dados.numero}
        maximo={NUMERO_ENDERECO_TAMANHO_MAXIMO}
        aoMudar={alterar("numero")}
        classe="sm:col-span-2"
        requerido
        placeholder="150 ou S/N"
      />
      <Campo
        rotulo="Complemento"
        nome="complemento"
        valor={dados.complemento ?? ""}
        maximo={COMPLEMENTO_TAMANHO_MAXIMO}
      ...
      {perderaConfirmacao && (
        <p
          data-aviso-confirmacao
          role="status"
          className="text-xs text-aviso sm:col-span-6"
        >
          Você mudou dados do endereço. Vamos pedir para confirmar de novo no
          mapa onde entregar.
        </p>
      )}
