...
export function ListaPedidosEmpresa({ pedidos, aoAbrir }: { pedidos: PedidoDaEmpresa[]; aoAbrir: (pedido: PedidoDaEmpresa) => void }) {
  if (pedidos.length === 0) return <p className="text-sm text-conteudo-suave">Nenhum pedido aqui.</p>;

  return (
    <ol aria-label="Pedidos" className="flex flex-col divide-y divide-borda rounded-jaa border border-borda text-sm">
      {pedidos.map((pedido) => (
        <li key={pedido.id} data-pedido={pedido.id} className="flex items-center justify-between gap-2 px-3 py-2">
          ...
          <button type="button" onClick={() => aoAbrir(pedido)} className="shrink-0 rounded-jaa border px-2 py-1 text-xs">
            Abrir
          </button>
        </li>
      ))}
