import * as z from "zod";

export const codigoErroApiSchema = z.enum([
  "NAO_AUTENTICADO",
  "DADOS_INVALIDOS",
  "NOME_USUARIO_INDISPONIVEL",
  "IDENTIDADE_PESSOAL_JA_EXISTE",
  "CADASTRO_INCOMPLETO",
  "IDENTIDADE_NAO_ENCONTRADA",
  "IDENTIDADE_NAO_AUTORIZADA",
  "EMPRESA_NAO_ENCONTRADA",
  "SLUG_INDISPONIVEL",
  "PRODUTO_NAO_ENCONTRADO",
  "CATEGORIA_NAO_ENCONTRADA",
  "CATEGORIA_NOME_EM_USO",
  "GRUPO_OPCOES_NAO_ENCONTRADO",
  "OPCAO_NAO_ENCONTRADA",
  "LIMITE_DE_GRUPOS_ATINGIDO",
  "LIMITE_DE_OPCOES_ATINGIDO",
  "ESCOLHAS_INVALIDAS",
  "ARQUIVO_INVALIDO",
  "ARMAZENAMENTO_INDISPONIVEL",
  "CREDENCIAIS_INVALIDAS",
  "SENHA_FRACA",
  "SENHA_JA_DEFINIDA",
  "ENDERECO_NAO_ENCONTRADO",
  "CEP_NAO_ENCONTRADO",
  "CEP_INDISPONIVEL",
  "LOCALIZACAO_NAO_CONFIRMADA",
  "ENDERECO_FORA_AREA_ENTREGA",
  "PEDIDO_NAO_ENCONTRADO",
  "TRANSICAO_PEDIDO_INVALIDA",
  "ENTREGADOR_NAO_ENCONTRADO",
  "ENTREGADOR_INATIVO",
  "ENTREGADOR_INDISPONIVEL",
  "SAIDA_NAO_ENCONTRADA",
  "PEDIDOS_INVALIDOS_PARA_SAIDA",
  "SEQUENCIA_DESATUALIZADA",
  "SAIDA_EM_ANDAMENTO",
  "BASE_NAO_CONFIGURADA",
  "ZONA_NAO_ENCONTRADA",
  "ZONA_INVALIDA",
  "ZONAS_SOBREPOSTAS",
  "SAIDA_NAO_ESTA_EM_FORMACAO",
  "SAIDA_NAO_ESTA_EM_ANDAMENTO",
  "SAIDA_NAO_LIBERADA",
  "SAIDA_NAO_PODE_SER_LIBERADA",
  "SAIDA_NAO_PODE_SER_RECUSADA",
  "LOCALIZACAO_IMPRECISA",
  "ENTREGA_NAO_ENCONTRADA",
  "ENTREGADOR_NAO_ATRIBUIDO",
  "ATRIBUICAO_CONFLITANTE",
  "ITENS_INVALIDOS",
  "PAGAMENTO_INVALIDO",
  "CONVERSA_CONSIGO_MESMO",
  "CONVERSA_NAO_ENCONTRADA",
  "ID_CLIENTE_REUTILIZADO",
  "MENSAGEM_NAO_ENCONTRADA",
  "MENSAGEM_RESPONDIDA_NAO_ENCONTRADA",
  "MENSAGEM_DE_OUTRA_IDENTIDADE",
  "MENSAGEM_EXCLUIDA",
]);

export type CodigoErroApi = z.infer<typeof codigoErroApiSchema>;

export const erroApiSchema = z.object({
  codigo: codigoErroApiSchema,
  mensagem: z.string(),
});

export type ErroApi = z.infer<typeof erroApiSchema>;
